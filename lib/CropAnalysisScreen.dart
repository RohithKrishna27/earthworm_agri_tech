  ...
  Future<void> _fetchDataFromFirebase() async {
    try {
      ...
      if (snapshot.snapshot.value != null) {
        ...
        setState(() {
        _nController.text = (data['n'] ?? 0).toString(); 
_pController.text = (data['p'] ?? 0).toString(); 
_kController.text = (data['k'] ?? 0).toString(); 
_temperatureController.text = (data['temperature'] ?? 0).toString() ;
_humidityController.text = (data['moisture'] ?? 0).toString(); 
_phController.text = (data['ph'] ?? 0).toString(); 
_rainfallController.text = (data['rainfall'] ?? 0).toString();// Default to 0 if not present
        });
      } else {
